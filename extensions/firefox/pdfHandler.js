...
var VIEWER_URL = (typeof browser !== 'undefined' ? browser : chrome).runtime.getURL("content/web/viewer.html");
...
browserAPI.storage.session.get({ hasPdfRedirector: false }, async items => {
  if (items?.hasPdfRedirector) {
    return;
  }
  const rules = await browserAPI.declarativeNetRequest.getDynamicRules();
  if (rules.length) {
  ...
  browserAPI.storage.session.set({ hasPdfRedirector: true });
});
...
browserAPI.runtime.onMessage.addListener(function (message, sender, sendResponse) {
  ...
  if (message && message.action === "isAllowedFileSchemeAccess") {
    browserAPI.extension.isAllowedFileSchemeAccess(sendResponse);
    return true;
  }
  if (message && message.action === "openExtensionsPageForFileAccess") {
    var url = "chrome://extensions/?id=" + browserAPI.runtime.id;
    if (message.data.newTab) {
      ...
      browserAPI.tabs.update(sender.tab.id, {
        url,
      });
    }
    return undefined;
