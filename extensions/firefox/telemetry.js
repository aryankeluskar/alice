...
(function () {
  ...
  if (browserAPI.runtime.id !== "oemmndcbldboiebfnladdacbdfmadadm") {
  ...
  // browserAPI.storage.local and use this "localStorage" object to enable
  ...
  browserAPI.storage.session.get({ didPingCheck: false }, async items => {
    if (items?.didPingCheck) {
      return;
    }
    maybeSendPing();
    await browserAPI.alarms.clear("maybeSendPing");
    await browserAPI.alarms.create("maybeSendPing", { periodInMinutes: 60 });
    browserAPI.storage.session.set({ didPingCheck: true });
  });
  ...
  function doSendPing() {
    browserAPI.storage.local.get(localStorage, items => {
  ...
  function getLoggingPref(callback) {
    ...
    function next(result) {
      ...
      if (!browserAPI.storage[storageAreaName]) {
        next();
        return;
      }

      browserAPI.storage[storageAreaName].get("disableTelemetry", function (items) {
